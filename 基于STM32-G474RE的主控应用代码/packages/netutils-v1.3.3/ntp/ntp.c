...
#define DBG_LEVEL                     DBG_INFO //DBG_DEBUG
#include <rtdbg.h>
...
//    // 如果 host_name 为空，使用默认的阿里云 NTP 域名
//    if (host_name == RT_NULL || strlen(host_name) == 0) {
//        host_name = "ntp.aliyun.com";
//    }
    ...
    *((char *) &packet + 0) = 0x1b;
    ...
    while (rt_tick_get() - start < NTP_GET_TIMEOUT * RT_TICK_PER_SECOND)
    {
        for (i = 0; i < server_num; i++)
        {
            ...
            {
                //LOG_E("Reading from server is(%d).", server_num);
                goto __exit;
            }
        }
