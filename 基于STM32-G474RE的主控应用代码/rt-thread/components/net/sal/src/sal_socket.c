...
struct sal_socket *sal_get_socket(int socket)
{
    ...
    /* 检查 socket 结构体是否有效 */
    if (st->sockets[socket] == RT_NULL || st->sockets[socket]->magic != SAL_SOCKET_MAGIC)
    {
        LOG_E("sal_get_socket: invalid socket %d (magic mismatch)", socket + SAL_SOCKET_OFFSET);
        return RT_NULL;
    }
