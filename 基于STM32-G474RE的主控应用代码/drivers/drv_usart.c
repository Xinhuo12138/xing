...
#define GPIO_AF5_UART5         ((uint8_t)0x05)  /* UART5 Alternate Function mapping      */
    ...
    /* STM32G4 系列：UART4/5 使用 AF5，其他 UART 按原有规则 */
    if (uart_num == 4 || uart_num == 5) {
        GPIO_InitStruct.Alternate = GPIO_AF5_UART4;   // UART4/5 复用功能 AF5
    } else if (uart_num <= 3) {
        GPIO_InitStruct.Alternate = GPIO_AF7;          // USART1/2/3 使用 AF7
    } else if (uart_num == 8) {
        GPIO_InitStruct.Alternate = GPIO_AF8;          // USART8 使用 AF8（若存在）
    } else {
        GPIO_InitStruct.Alternate = GPIO_AF12_LPUART1; // LPUART1 使用 AF12
    }
